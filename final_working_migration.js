...
async function runFinalMigration() {
  try {
    console.log('Running final working migration...');
    ...
    // Sample plans with required fields (without end_date)
    const samplePlans = [
      {
        plan_number: 'PLAN003',
    ...
    const { data: plans, error: selectError } = await supabase
      .from('installment_plans')
      .select('plan_number, name, months, number_of_installments, total_amount, installment_amount, status, requires_guarantor')
      .order('plan_number');

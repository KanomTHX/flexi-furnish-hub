-- Enhanced Transfer System - Working Version
-- This version uses simplified RLS policies that work with any authentication setup
...
-- ============================================================================
-- 2. CREATE PRODUCT SERIAL NUMBERS TABLE
-- ============================================================================
...
-- ============================================================================
-- 4. CREATE STOCK TRANSFER ITEMS TABLE
-- ============================================================================
...
-- ============================================================================
-- 6. CREATE SIMPLE RLS POLICIES (WORKING VERSION)
-- ============================================================================
...
-- Simplified warehouse system policies (allow all authenticated users for now)
...
DECLARE
    ...
    i INTEGER;
BEGIN
    -- Get first active warehouse and product
    ...
    IF sample_warehouse_id IS NOT NULL AND sample_product_id IS NOT NULL THEN
        -- Insert 5 sample serial numbers
        FOR i IN 1..5 LOOP
            ...
            VALUES (
                'TEST-' || EXTRACT(EPOCH FROM NOW())::bigint || '-' || i,
                sample_product_id,
                sample_warehouse_id,
                1000 + (i * 100),
                'available'
        ...
        RAISE NOTICE 'Sample serial numbers created for testing';
    ELSE
        RAISE NOTICE 'No active warehouse or product found for sample data';
    END IF;
END $$;
...
SELECT 'Enhanced Transfer System setup completed successfully!' as result,
       'Tables created: notifications, product_serial_numbers, stock_transfers, stock_transfer_items' as tables_created,
       'Sample data inserted for testing' as sample_data,
       'Run: node scripts/test-system-after-setup.js' as next_step;
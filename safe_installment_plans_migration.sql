-- ===================================================================
-- SAFE MIGRATION FOR INSTALLMENT_PLANS
-- แก้ไขตามโครงสร้างตารางจริง (ปลอดภัย)
-- ===================================================================
...
-- 3. ดึง branch_id แรกที่มีอยู่ (หรือใช้ค่า default)
DO $$
DECLARE
...
BEGIN
    ...
    -- ถ้าไม่มี branches ให้ใช้ UUID default
    IF default_branch_id IS NULL THEN
        default_branch_id := '00000000-0000-0000-0000-000000000000'::uuid;
    END IF;
    
    -- เพิ่มข้อมูลตัวอย่างแผนผ่อนชำระ
    ...
    SELECT * FROM (VALUES
        ('PLAN003', 'ผ่อน 0% 3 งวด', 3, 0.00, 1000.00, 10.00, 200.00, 'ผ่อนชำระ 3 งวด ไม่มีดอกเบี้ย', 3000, 30000, FALSE, TRUE, 'active', default_branch_id),
        ('PLAN006', 'ผ่อน 0% 6 งวด', 6, 0.00, 2000.00, 20.00, 500.00, 'ผ่อนชำระ 6 งวด ไม่มีดอกเบี้ย', 5000, 50000, FALSE, TRUE, 'active', default_branch_id),
        ('PLAN012', 'ผ่อน 0% 12 งวด', 12, 0.00, 3000.00, 30.00, 1000.00, 'ผ่อนชำระ 12 งวด ไม่มีดอกเบี้ย', 10000, 100000, FALSE, TRUE, 'active', default_branch_id),
        ('PLAN024', 'ผ่อน 8% 24 งวด', 24, 8.00, 5000.00, 25.00, 2000.00, 'ผ่อนชำระ 24 งวด ดอกเบี้ย 8% ต่อปี', 20000, 500000, TRUE, TRUE, 'active', default_branch_id),
        ('PLAN036', 'ผ่อน 10% 36 งวด', 36, 10.00, 10000.00, 30.00, 3000.00, 'ผ่อนชำระ 36 งวด ดอกเบี้ย 10% ต่อปี', 50000, 1000000, TRUE, TRUE, 'active', default_branch_id)
    ...
    RAISE NOTICE 'Added installment plans with branch_id: %', default_branch_id;
END $$;
...
-- เสร็จสิ้น
SELECT 'Safe installment_plans migration completed!' as result;
-- ===================================================================
-- FINAL INSTALLMENTS MIGRATION
-- การอัปเกรดระบบสัญญาผ่อนชำระ (ตรวจสอบแล้ว)
-- ===================================================================

-- Migration: 20241208_installment_system_final_upgrade
...
-- เพิ่ม unique constraint สำหรับเลขบัตรประชาชน (ถ้ายังไม่มี)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- เพิ่ม foreign key สำหรับ guarantor (ถ้ายังไม่มี)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- เพิ่ม foreign key สำหรับ contract_history (ถ้ายังไม่มี)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- เพิ่ม foreign key สำหรับ contract_documents
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_documents_contract' 
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_documents_guarantor' 
        AND table_name = 'contract_documents'
    ) THEN
        ALTER TABLE contract_documents 
        ADD CONSTRAINT fk_documents_guarantor 
        FOREIGN KEY (guarantor_id) REFERENCES guarantors(id) ON DELETE CASCADE;
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_documents_customer' 
        AND table_name = 'contract_documents'
    ) THEN
        ALTER TABLE contract_documents 

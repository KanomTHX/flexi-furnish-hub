-- Insert mock data for testing (corrected UUID format)
...
-- Get branch and category IDs for use in other inserts
WITH branch_data AS (
  SELECT id as branch_id, name as branch_name FROM branches ORDER BY created_at LIMIT 3
),
category_data AS (
  SELECT id as category_id, name as category_name FROM product_categories ORDER BY created_at LIMIT 5
),
...
product_inserts AS (
  ...
  SELECT 
    ...
    NOW()
  FROM (VALUES
    ('เก้าอี้สำนักงานผู้บริหาร', 'OfficeComfort', 'OC-001', 'เก้าอี้สำนักงานผู้บริหารแบบเออร์โกโนมิกส์ พร้อมพนักพิงหลัง', 'เฟอร์นิเจอร์สำนักงาน', 12500, 8000),
    ('ชุดโต๊ะอาหาร (4 เก้าอี้)', 'DiningSet', 'DT-205', 'โต๊ะอาหารไม้แท้ พร้อมเก้าอี้ 4 ตัวแบบเข้าชุด', 'ห้องอาหาร', 25900, 18000),
    ('ชั้นหนังสือพรีเมียม 5 ชั้น', 'BookShelf', 'BS-108', 'ชั้นหนังสือไม้คุณภาพสูง 5 ชั้น ปรับระดับได้', 'ที่เก็บของ', 8900, 6000),
    ('โซฟา 3 ที่นั่ง ผ้า', 'SofaFlex', 'SF-301', 'โซฟาผ้า 3 ที่นั่ง นั่งสบาย ดีไซน์โมเดิร์น', 'ห้องนั่งเล่น', 35000, 25000),
    ('โต๊ะกาแฟหน้ากระจก', 'CoffeeTable', 'CT-150', 'โต๊ะกาแฟหน้ากระจกโมเดิร์น โครงเหล็ก', 'ห้องนั่งเล่น', 15500, 11000)
  ) AS product_data(name, brand, model, description, category_name, base_price, cost_price)
  LEFT JOIN category_data c ON c.category_name = product_data.category_name
  RETURNING id, name, model
)
-- Insert basic data first, then we'll add more complex relationships
SELECT 'Products inserted' as result;
...
-- Insert sample claims with generated serial numbers
...
(gen_random_uuid(), 'CLM-2024-001', 'นายสมชาย ใจดี', '081-234-5678', 'SF001234', 'defect', 'โซฟามีปัญหาเบาะยุบและเสียงดังเมื่อนั่ง อาจเป็นปัญหาจากสปริงภายใน', 'in_progress', 2500, '2024-01-15', 'ตรวจสอบแล้วพบว่าสปริงชำรุด', NOW(), NOW()),
...
(gen_random_uuid(), 'CLM-2024-004', 'นายสมชาย ใจดี', '081-234-5678', 'OC001234', 'installation', 'เก้าอี้ไม่สามารถประกอบได้ เนื่องจากขาดน็อตและสกรู', 'approved', 500, '2024-01-22', 'อนุมัติแล้ว รอจัดส่งอะไหล่', NOW(), NOW());
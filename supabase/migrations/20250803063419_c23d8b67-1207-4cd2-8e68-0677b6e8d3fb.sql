-- First, create sample employees for references

-- Insert sample employees with positions and departments
WITH department_sales AS (
  SELECT id FROM departments WHERE name = 'ฝ่ายขาย' LIMIT 1
),
position_sales AS (
  SELECT id FROM positions WHERE name = 'พนักงานขาย' LIMIT 1
),
sample_employees AS (
  INSERT INTO employees (
    id, employee_id, first_name, last_name, email, phone, address, 
    date_of_birth, hire_date, position_id, department_id, salary, status,
  ...
  SELECT 
    ...
    gen_random_uuid(), -- created_by
    gen_random_uuid(), -- updated_by
    NOW(),
    NOW()
  FROM (VALUES
    ('สมชาย', 'ขยันขาย', 'somchai.sales@company.com', '081-111-1111', '123 ถนนสุขุมวิท กรุงเทพฯ', '1990-01-15'::date, '2023-01-01'::date, 18000),
    ('มาลี', 'รักงาน', 'malee.work@company.com', '082-222-2222', '456 ถนนพหลโยธิน กรุงเทพฯ', '1992-05-20'::date, '2023-03-01'::date, 18000),
    ('สมหญิง', 'ใจดี', 'somying.good@company.com', '083-333-3333', '789 ถนนรัชดาภิเษก กรุงเทพฯ', '1988-08-10'::date, '2022-06-01'::date, 20000)
  ) AS emp_data(first_name, last_name, email, phone, address, date_of_birth, hire_date, salary)
  CROSS JOIN position_sales pos
  CROSS JOIN department_sales dept
  RETURNING id, first_name, last_name
),
-- Get branch and first employee for references
branch_data AS (
...
-- Insert installment sales transactions
...
contract_inserts AS (
  ...
  RETURNING id, contract_number, total_months, monthly_payment, start_date, transaction_id
)
...
SELECT 
  ...
  END as late_fee,
  CASE 
    WHEN payment_month <= 2 THEN 'completed'::payment_status
    WHEN payment_month = 3 AND random() > 0.5 THEN 'completed'::payment_status

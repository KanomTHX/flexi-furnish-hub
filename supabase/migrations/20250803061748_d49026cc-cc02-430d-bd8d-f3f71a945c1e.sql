-- Insert mock data for installment contracts and payments
...
product_data AS (
  SELECT id, name, base_price FROM products WHERE base_price >= 20000 LIMIT 3
),
...
installment_transactions AS (
  ...
  SELECT 
    ...
    'installment_sale'::transaction_type,
...
-- Insert sample sale items for installment transactions
INSERT INTO sale_items (id, transaction_id, product_id, serial_number, quantity, unit_price, line_total, created_at)
SELECT 
  gen_random_uuid(),
  st.id,
  p.id,
  p.model || LPAD((ROW_NUMBER() OVER())::text, 4, '0') as serial_number,
  1,
  p.base_price,
  p.base_price,
  NOW()
FROM installment_transactions st
JOIN products p ON (
  (st.customer_name = 'นายสมชาย รวยเร็ว' AND p.name = 'โซฟา 3 ที่นั่ง ผ้า') OR
  (st.customer_name = 'นางสาวมาลี ชอบช้อป' AND p.name = 'ชุดโต๊ะอาหาร (4 เก้าอี้)') OR
  (st.customer_name = 'บริษัท ออฟฟิศเซ็ต จำกัด' AND p.name = 'เก้าอี้สำนักงานผู้บริหาร')
)
LIMIT 10;
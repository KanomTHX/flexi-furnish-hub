-- ===================================================================
-- MINIMAL INSTALLMENT PLANS MIGRATION
-- เวอร์ชันเรียบง่าย - ใส่เฉพาะคอลัมน์ที่จำเป็นจริงๆ
-- ===================================================================

-- จากการตรวจสอบพบว่าคอลัมน์ที่จำเป็น (NOT NULL) มีเพียง:
-- 1. plan_number (VARCHAR)
...
-- 2. เพิ่มข้อมูลตัวอย่างแผนผ่อนชำระ (ใส่เฉพาะคอลัมน์ที่จำเป็น + คอลัมน์ที่มีอยู่)
DO $$
DECLARE
...
BEGIN
    ...
    -- เพิ่มข้อมูลตัวอย่าง (ใส่เฉพาะคอลัมน์ที่จำเป็นและคอลัมน์ที่มีอยู่)
    ...
    SELECT * FROM (VALUES
        ('PLAN003', 10000.00, 'ผ่อน 0% 3 งวด', 3, 0.00, 1000.00, 10.00, 200.00, 'ผ่อนชำระ 3 งวด ไม่มีดอกเบี้ย', 3000, 30000, FALSE, TRUE, 3333.33, 'active', default_branch_id),
        ('PLAN006', 15000.00, 'ผ่อน 0% 6 งวด', 6, 0.00, 2000.00, 20.00, 500.00, 'ผ่อนชำระ 6 งวด ไม่มีดอกเบี้ย', 5000, 50000, FALSE, TRUE, 2166.67, 'active', default_branch_id),
        ('PLAN012', 50000.00, 'ผ่อน 0% 12 งวด', 12, 0.00, 3000.00, 30.00, 1000.00, 'ผ่อนชำระ 12 งวด ไม่มีดอกเบี้ย', 10000, 100000, FALSE, TRUE, 3916.67, 'active', default_branch_id),
        ('PLAN024', 100000.00, 'ผ่อน 8% 24 งวด', 24, 8.00, 5000.00, 25.00, 2000.00, 'ผ่อนชำระ 24 งวด ดอกเบี้ย 8% ต่อปี', 20000, 500000, TRUE, TRUE, 4583.33, 'active', default_branch_id),
        ('PLAN036', 200000.00, 'ผ่อน 10% 36 งวด', 36, 10.00, 10000.00, 30.00, 3000.00, 'ผ่อนชำระ 36 งวด ดอกเบี้ย 10% ต่อปี', 50000, 1000000, TRUE, TRUE, 6111.11, 'active', default_branch_id)
    ) AS new_plans(plan_number, total_amount, name, months, interest_rate, down_payment, down_payment_percent, processing_fee, description, min_amount, max_amount, requires_guarantor, is_active, installment_amount, status, branch_id)
    WHERE NOT EXISTS (
...
-- เสร็จสิ้น
SELECT 'Minimal installment_plans migration completed successfully!' as result;